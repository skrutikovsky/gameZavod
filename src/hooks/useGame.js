...
export function useGame() {
  ...
  const updateBoxes = useCallback((deltaTime) => {
    ...
    boxesRef.current.forEach((box, index) => {
      ...
      // ВАЖНО: проверяем только когда коробка ТОЧНО пересекает линию снизу вверх
      // и ещё не была зафиксирована или пропущена
      if (!box.checked && boxBottom >= effectiveStopY) {
        box.checked = true;
