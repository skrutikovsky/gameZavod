...
export function useGame2() {
  ...
  const updateBoxes = useCallback(() => {
    ...
    // Позиция линии остановки (проверяется только нижняя граница коробки)
    ...
    // Если рука активна - линия становится физической преградой для НИЖНЕЙ ГРАНИЦЫ коробок
    if (isHandBlocking) {
      ...
      for (let i = boxesRef.current.length - 1; i >= 0; i--) {
        ...
        // Коробка останавливается ТОЛЬКО если её нижняя граница достигает или пересекает линию остановки
        // И при этом верх коробки ещё выше линии (коробка не прошла линию полностью)
        // Если коробка уже полностью ниже линии (box.y >= handStopLineY), она НЕ может быть остановлена
        if (boxBottom >= handStopLineY && box.y < handStopLineY) {
          // Останавливаем коробку так чтобы её низ был точно на линии
          box.y = handStopLineY - boxHeightPercent;
          box.stopped = true;
        }
        
        // Проверяем столкновение с коробкой ниже
        // Коробка может остановиться от другой коробки только если:
        // 1. Нижняя коробка остановлена
        // 2. Нижняя коробка находится выше или на линии остановки (её низ <= линии)
        // 3. Текущая коробка ещё не остановлена
