    ...
    if (isHandBlocking) {
      ...
      for (let i = boxesRef.current.length - 1; i >= 0; i--) {
        ...
        if (i === boxesRef.current.length - 1) {
          ...
          // и только в следующем кадре после пересечения
          if (boxBottom >= handStopLineY) {
            box.stopped = true;
            // Не фиксируем позицию резко, просто останавливаем движение
            // Позиция будет обновлена естественно в следующем кадре движения
          }
